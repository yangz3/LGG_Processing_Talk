...
- (void)write {
    if(self.connectedCharacteristic == nil){
        return;
    }
    NSString* str = @"Hi there!";
    NSData* data = [str dataUsingEncoding:NSUTF8StringEncoding];
    [self.connectedPeripheral writeValue:data forCharacteristic:self.connectedCharacteristic
...
- (void)centralManager:(CBCentralManager *)central
 didDiscoverPeripheral:(CBPeripheral *)peripheral
  ...
  if (!foundSuitablePeripheral) {
    ...
    if([peripheral.name isEqual:@"Mi's LG Watch"]){
        NSLog(@"Found you at last!");
...
didWriteValueForCharacteristic:(CBCharacteristic *)characteristic
    ...
    NSLog(@"didWriteValueForCharacteristic: Wrote data successfully!");
}
...
- (void)peripheral:(CBPeripheral *)peripheral
  ...
  for (CBCharacteristic *characteristic in service.characteristics) {
    ...
    if([self.characteristicUUIDs containsObject:characteristic.UUID]){
        LXCBLog(@"Found you!");
        //[peripheral writeValue:(NSData *)@"Hi there!" forCharacteristic:characteristic
        //                type:CBCharacteristicWriteWithResponse];
